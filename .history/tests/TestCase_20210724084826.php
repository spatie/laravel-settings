...
class TestCase extends BaseTestCase
{
    ...
    protected function migrateDummySettingsEloquent(
        string $fname = 'Louis',
        string $lanme = 'Armstrong',
        string $description = 'Hello Dolly'
    ): self {
        resolve(SettingsMigrator::class)->inGroup('dummy_simple', function (SettingsBlueprint $blueprint)
            use ($description, $fname, $lanme): void {
            $blueprint->add('fname', $fname);
            $blueprint->add('lanme', $lanme);
            $blueprint->add('description', $description);
        });
